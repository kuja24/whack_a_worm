...
var wormsX = [160,396,676,140,450,752,284,712];
var wormsY = [20,92,61, 198, 245, 173, 380,335 ];
...
function Main()
{
	...
	scoreImg.name = 'scoreBg';
	scoreImg.onload = loadGfx;
	...
	/* Ticker
	instead of using set time out setinterval animation frames etc
	ticker is a class in easel js which has all these mechanisms */
...
{	
	...
	playBtn.x = centerX - 20;
	playBtn.y = centerY + 270;
	
	creditsBtn.x = centerX + 300;
	creditsBtn.y = centerY + 270;
...
function showCredits()
{
	...
	creditsView.scaleX = 0.8;
	creditsView.scaleY = 0.8;
	creditsView.x = -203;
	//if using tween, no need to update stage
	Tween.get(creditsView).to({x:0}, 1000).call(function(){creditsView.onPress = hideCredits;});
}
...
function hideCredits()
{
	...
	Tween.get(creditsView).to({x:-203}, 1000).call(function(){creditsView.onPress = null; stage.removeChild(creditsView); creditsView = null;});
}

function showGameView()
{
	scoreBg = new Bitmap(scoreImg);
	...
	score = new Text('0' + '/' + totalWorms, 'bold 25px Charcoal', '#111');
	score.maxWidth = 1000;	
	score.x = 200;
	score.y = 40;
...
function showAlert()
{
	...
	Tween.get(alertBg).to({y:centerY - 80}, 200).call(function()
	{
		...
		var score = new Text(wormsHit + '/' + totalWorms, 'bold 50px Charcoal', '#EEE');
